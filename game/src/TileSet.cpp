...
namespace jumper
{
          ...
          m_tilesPerRow(tilesPerRow), m_numRows(numRows), m_tileOffset(tileOffset)
    {
        ...
        if (!tilesDataset.empty())
        {
            m_tiles = io.TileSetIO::template load<TileArray>("tiles", tilesDataset, m_tileWidth, m_tileHeight);
    ...
    TileSet::TileSet(MainWindow *mw, std::string filename) : StaticRenderable(mw)
    {
        ...
        /*m_tiles = new TileArray(0,
                                ...
                                m_tileWidth,
                                m_tileHeight);*/
        // Corresponding to old representation
    ...
    void TileSet::render()
    {
        if (readyToRender())
        {
            ...
            {
                // Draw full texture as background (no tile dataset)
                SDL_Rect src;
                SDL_Rect dst;
                ...
                SDL_RenderCopy(m_mainWindow->renderer(), m_texture, &src, &dst);
            }
        }
    }
